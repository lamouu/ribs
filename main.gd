...
func _ready() -> void:
	...
	spawn_buffer_y = (player_camera.size.y / 3)
	spawn_buffer_x = (player_camera.size.x / 3)
...
func _on_spawn_timer_timeout() -> void:
	...
	var player_camera_top_position = camera_center.y - (screen_size.y / 2)
	var player_camera_bottom_position = camera_center.y + (screen_size.y / 2)
	var map_leftmost_position = $Map.position.x
	var map_rightmost_position = map_leftmost_position + $Map.size.x
	var player_camera_leftmost_position = camera_center.x - (screen_size.x / 2)
	var player_camera_rightmost_position = camera_center.x + (screen_size.x / 2)
